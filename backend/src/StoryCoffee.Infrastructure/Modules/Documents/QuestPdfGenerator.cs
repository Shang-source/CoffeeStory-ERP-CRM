...
""";
    ...
    private byte[] GenerateInvoice(PdfDocumentResult result)
    {
        ...
        return Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Element(content => HeroHeader(content, invoice.CustomerName, "", invoice.AmountDue, invoice.DueDate));
                page.Content().PaddingTop(10).Column(column =>
                {
                    ...
                    column.Item().Element(content => DocumentSummary(content, [
                        ...
                        ("Issue", DateDmy(invoice.IssueDate)),
                        ("Due", DateDmy(invoice.DueDate)),
    ...
    private byte[] GenerateStatement(PdfDocumentResult result)
    {
        ...
        return Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Element(content => HeroHeader(content, statement.CustomerName, "", statement.TotalOutstanding, statement.StatementDate));
                page.Content().PaddingTop(10).Column(column =>
                {
                    column.Spacing(9);
                    ...
                    column.Item().Element(content => DocumentSummary(content, [
                        ...
                        ("Issue", DateDmy(statement.StatementDate)),
                        ("Period", $"{DateDmy(statement.PeriodStart)} - {DateDmy(statement.PeriodEnd)}"),
                        ("GST number", statement.Company.GstNumber),
                        ("Bill to", statement.CustomerName),
                        ("Billing address", statement.BillingAddress)
                    ]));

                    column.Item().Text("Account activity").FontSize(13).Bold().FontColor(TextColor);
                    column.Item().Element(content => StatementLedgerTable(content, statement.LedgerLines));
    ...
    private static void HeroHeader(IContainer container, string companyName, string documentTitle, decimal amountDue, DateTimeOffset dueDate)
    {
        container.Background(HeaderBackground).PaddingVertical(10).Column(hero =>
        {
            ...
            hero.Item().AlignCenter().Text($"Due on {DateDmy(dueDate)}").FontSize(10);
        });
    }
    ...
    private static void BrandBlock(IContainer container, CompanyDocumentProfile company)
    {
        container.Row(row =>
        {
            row.ConstantItem(120).Column(brand =>
            {
                brand.Spacing(1);
                brand.Item().Width(52).Svg(StoryCoffeeLogoSvg);
                brand.Item().Text("STORY").FontSize(17).Bold().FontColor(TextColor);
                brand.Item().Text("COFFEE").FontSize(9).FontColor(Colors.Grey.Darken1);
            });
            row.RelativeItem().AlignRight().Column(companyBlock =>
            {
                companyBlock.Spacing(1);
                companyBlock.Item().AlignRight().Text(company.Name).FontSize(10).Bold();
                companyBlock.Item().AlignRight().Text(company.PostalAddressLine1).FontSize(8);
                companyBlock.Item().AlignRight().Text(company.PostalAddressLine2).FontSize(8);
                companyBlock.Item().AlignRight().Text(company.Country).FontSize(8);
                companyBlock.Item().AlignRight().Text(company.Website).FontSize(8);
            });
        });
    }
    ...
    private static void StatementItemsTable(IContainer container, IReadOnlyList<StatementInvoicePdfLine> invoices)
    {
        container.Table(table =>
        {
            ...
            TableHeader(table, "Issue", "Due", "Invoice", "Status", "Outstanding");
            foreach (var invoice in invoices)
            {
                table.Cell().Element(TableCell).Text(DateDmy(invoice.IssueDate));
                table.Cell().Element(TableCell).Text(DateDmy(invoice.DueDate));
    ...
    private static void StatementLedgerTable(IContainer container, IReadOnlyList<StatementLedgerPdfLine> lines)
    {
        container.Table(table =>
        {
            table.ColumnsDefinition(columns =>
            ...
            TableHeader(table, "Issue", "Due", "Description", "Debit", "Credit", "Balance");
            foreach (var line in lines)
            {
                table.Cell().Element(TableCell).Text(DateDmy(line.IssueDate));
                table.Cell().Element(TableCell).Text(DateDmy(line.DueDate));
                table.Cell().Element(TableCell).Text(line.Description);
                table.Cell().Element(TableCell).AlignRight().Text(line.Debit.HasValue ? Money(line.Debit.Value) : "");
                table.Cell().Element(TableCell).AlignRight().Text(line.Credit.HasValue ? Money(line.Credit.Value) : "");
                table.Cell().Element(TableCell).AlignRight().Text(Money(line.Balance));
            }
        });
    }
    ...
    private static void InvoiceClosingRow(IContainer container, InvoicePdfDocument invoice)
    {
        container.Row(row =>
        {
            row.RelativeItem().Element(content => PaymentNotes(content, invoice.Company, invoice.AccountNumber));
            row.ConstantItem(16);
    ...
    private static void StatementClosingRow(IContainer container, StatementPdfDocument statement)
    {
        container.Row(row =>
        {
            row.RelativeItem().Element(content => PaymentNotes(content, statement.Company, statement.AccountNumber, includeAccountName: true));
            row.ConstantItem(16);
    ...
    private static void PaymentNotes(IContainer container, CompanyDocumentProfile company, string reference, bool includeAccountName = false)
    {
        container.Background(NotesBackground).Padding(9).Column(column =>
        {
            column.Spacing(2);
            column.Item().Text("Notes").FontSize(10).Bold();
            column.Item().Text(includeAccountName
                ? $"Please use {reference} as the reference."
                : $"Please use your company name or {reference} as the reference.");
            if (includeAccountName)
            {
                column.Item().PaddingTop(4).Text($"Account name: {company.BankAccountName}");
            }
    ...
    private static string DateDmy(DateTimeOffset date)
    {
    ...
    private static string DateDmy(DateTimeOffset? date)
    {
        return date.HasValue ? DateDmy(date.Value) : "-";
    }
}
