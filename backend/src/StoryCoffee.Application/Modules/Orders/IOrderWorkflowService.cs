...
{
    Task<IReadOnlyList<OrderDto>> GetAdminOrders(OrderQueryRequest query, CancellationToken cancellationToken);
    ...
    Task<BatchShipAndInvoiceResponse> BatchShipAndInvoice(IReadOnlyList<Guid> orderIds, Guid actorUserId, CancellationToken cancellationToken);
